...
CREATE CONTINUOUS QUERY "cq_devices_30m" ON "sensors" BEGIN SELECT mean("Temperature") AS "Temperature" INTO "autogen"."devices" FROM "m"."devices" GROUP BY time(30m),* END
...
CREATE CONTINUOUS QUERY "cq_temperature_30m" ON "sensors" BEGIN SELECT mean("value") as "value" INTO "autogen"."temperature" FROM "m"."temperature" GROUP BY time(30m),* END
CS_EOF
}
