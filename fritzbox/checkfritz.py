#!/usr/bin/env python3
# https://github.com/blackw1ng/FritzBox-monitor
...
try:
    fc = FritzConnection(address='fritz.box', user=os.environ['FRITZ_USERNAME'], password=os.environ['FRITZ_PASSWORD'], timeout=2.0)

except BaseException as e:
    print("Cannot connect to fritzbox.", e)
    sys.exit(1)
...
# tag every measurement by fritzbox serial number
...
#if IS_DSL:
#  info = readout('WANPPPConnection1', 'GetInfo')
#else:
info = readout('WANIPConn1', 'GetStatusInfo')
traffic = readout('WANCommonIFC1', 'GetAddonInfos')
# 7412 KeyError: NewX_AVM_DE_TotalBytesSent64
